...
def _update_output_monitoring(parameters:dict,prev_task_result):
    ...
    entity_path = f'{parameters["OutputPath"]}/{parameters["DagId"]}/{parameters["DateDir"]}/ROLLING_VOLUMES_FDM.parquet'
    ...
    df = pd.DataFrame([{'PipelineRunId': parameters["RunId"],
                        'EntityName':'ROLLING_VOLUMES_FDM.parquet',
...
with DAG(
    dag_id='jupiter_rolling_volumes_fdm',
