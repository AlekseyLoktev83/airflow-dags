...
def get_parameters(**kwargs):
    ...
    raw_path = Variable.get("RawPath#PTW")
    process_path = Variable.get("ProcessPath#PTW")
    output_path = Variable.get("OutputPath#PTW")
    white_list = Variable.get("WhiteList#PTW", default_var=None)
    black_list = Variable.get("BlackList#PTW", default_var=None)
...
def update_last_upload_date(last_upload_date):
    ...
    conn.secrets.kv.v2.create_or_update_secret(
        path="variables/LastUploadDate#PTW", secret={"value": last_upload_date})


with DAG(
    dag_id='ptw_raw_data_upload',
